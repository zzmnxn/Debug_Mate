...
// AI 파싱 강화를 위한 헬퍼 함수
async function enhancedAIParsing(
  query: string,
  context: string = ""
...
Output the analysis result in JSON format only:`;
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      if (
        parsed.tool &&
        ["loopCheck", "traceVar", "testBreak", "afterDebugFromCode"].includes(
          parsed.tool
        )
      ) {
        return {
...
async function parseSingleIntent(query: string): Promise<ParsedIntent> {
  ...
  const orderPatterns = [
    {
      keywords: ["첫", "첫번째", "첫 번째", "1번째", "하나번째", "처음"],
      target: "first",
    },
    {
      keywords: ["두", "둘", "두번째", "두 번째", "2번째", "둘째", "이번째"],
      target: "second",
    },
    {
      keywords: ["세", "셋", "세번째", "세 번째", "3번째", "셋째", "삼번째"],
      target: "third",
    },
    {
      keywords: ["네", "넷", "네번째", "네 번째", "4번째", "넷째", "사번째"],
      target: "fourth",
    },
    {
      keywords: ["다섯", "다섯번째", "다섯 번째", "5번째", "오번째"],
      target: "fifth",
    },
    {
      keywords: ["마지막", "마지막번째", "끝", "마지막거", "라스트"],
      target: "last",
    },
  ];
  ...
  const loopTypePatterns = [
    {
      keywords: ["for문", "for루프", "for반복문", "포문", "for"],
      loopType: "for",
    },
    ...
    {
      keywords: ["while문", "while루프", "while반복문", "와일문", "while"],
      loopType: "while",
    },
  ];
  ...
  const hasOverallAnalysis = flexibleMatch(
    normalizedQuery,
    overallAnalysisKeywords
  );
    ...
    try {
      ...
      if (parsedIntents.isMultiple) {
        ...
        if (
          ...
          // 루프 비교 요청인 경우
          const targets = parsedIntents.intents.map(
            (intent) => intent.target || "all"
          );
