# 루트에서 CLI 사용
npm run debug-mate test.c              # tmux 분할 화면으로 디버깅
npm run debug-mate debug test.c        # 명시적 debug 명령어
npm run debug-mate tmux test.c         # tmux 명령어 (debug와 동일)
npm run debug-mate generate            # 테스트 코드 생성
npm run debug-mate status              # 시스템 상태 확인
npm run debug-mate info                # 프로그램 정보

# 또는 직접 CLI 실행
node cli/debug-mate-cli.js test.c      # 기본 디버깅
node cli/debug-mate-cli.js debug test.c # 명시적 debug
node cli/debug-mate-cli.js generate    # 테스트 코드 생성
